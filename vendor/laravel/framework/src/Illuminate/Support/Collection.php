...
class Collection implements ArrayAccess, Arrayable, Countable, IteratorAggregate, Jsonable, JsonSerializable
{
    ...
    /**
     * @author zhanglingzhi
     * 之前小林封装过，为了方便大家使用，继续复制过来
     */
        public function mapBy($mapKey, $valueKey = null)
    {
        $results = array();

        foreach ($this->items as $key => $value)
        {
            $key = is_callable($mapKey) ? $mapKey($value, $key) : data_get($value, $mapKey);

            if(! is_null($valueKey)) $results[$key] = $value[$valueKey];
